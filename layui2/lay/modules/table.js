...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  ,TPL_HEADER = function(options){
    ...
    options = options || {};
    return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table"   id="tab3"'
          ...
          ,'<th data-field="{{ item2.field||i2 }}" data-key="{{d.index}}-{{i1}}-{{i2}}" {{# if( item2.parentKey){ }}data-parentkey="{{ item2.parentKey }}"{{# } }} {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize || item2.colGroup){ }}data-unresize="true"{{# } }} class="{{# if(item2.hide){ }}layui-hide{{# } }}{{# if(isSort){ }} layui-unselect{{# } }}{{# if(!item2.field){ }} layui-table-col-special{{# } }}">'
            ...
            ,'" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>'
              ,'{{# if(item2.type === "checkbox"){ }}' //复选框
                ,'<input type="checkbox" id="ch" name="layTableCheckbox" lay-skin="primary" lay-filter="layTableAllChoose" {{# if(item2[d.data.checkName]){ }}checked{{# }; }}>'
              ,'{{# } else { }}'
  ...
  //tbody区域模板
  ,TPL_BODY = ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" id="ttt"'
    ,'{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>'
    ,'<tbody></tbody>'
  ,'</table>'].join('')
    ...
    ,render = function(){ //后续性能提升的重点
      ...
      layui.each(data, function(i1, item1){
        ...
        that.eachCols(function(i3, item3){
          ...
          //td内容
          var td = ['<td  id="ttq" data-field="'+ field +'" data-key="'+ key +'" '+ function(){ //追加各种属性
            var attr = [];
        ...
        trs.push('<tr id="tr1" data-index="'+ i1 +'">'+ tds.join('') + '</tr>');
        trs_fixed.push('<tr  id="tr2" data-index="'+ i1 +'">'+ tds_fixed.join('') + '</tr>');
        trs_fixed_r.push('<tr  id="tr3" data-index="'+ i1 +'">'+ tds_fixed_r.join('') + '</tr>');
      });
  ...
  Class.prototype.renderTotal = function(data){
    ...
    that.eachCols(function(i3, item3){
      ...
      //td内容
      var td = ['<td  id="tdd" data-field="'+ field +'" data-key="'+ options.index + '-'+ item3.key +'" '+ function(){
        var attr = [];
        if(item3.align) attr.push('align="'+ item3.align +'"'); //对齐方式
        if(item3.style) attr.push('style="'+ item3.style +'"'); //自定义样式
        if(item3.minWidth) attr.push('data-minwidth="'+ item3.minWidth +'"'); //单元格最小宽度
  ...
  Class.prototype.events = function(){
        ...
        case 'LAYTABLE_EXPORT': //导出
          if(device.ie){
            ...
            openPanel({
              list: function(){
                return [
                  '<li data-type="csv1">导出到 Csv1 文件</li>'
                  ,'<li data-type="xls1">导出到 Excel1 文件</li>'
                ].join('')
              }()
    ...
    that.layBody.on('change', '.'+ELEM_EDIT, function(){
      ...
      ,data = table.cache[that.key][index];
			var wpw=$('#sel').val();
			wpw=parseInt(wpw)*(resc-1)+index;
			alert("f:"+field+" "+wpw+" "+value)
		  switch(field){
				case "tel":
				var reg=/^1\d{10}$/;
				 if(!reg.test(value)){
      ...
      console.log("y1:",field," ",value," ",index," ",ELEM_EDIT)
